...
class Measurement(tk.Frame):
    ...
    def _instContTurnOn(self):
        self.controller.instrument.outp_ON('a')
        self.controller.instrument.outp_ON('b')
        # print('unsubscribing measurement')

    def _instContTurnOff(self):
        self.controller.instrument.outp_OFF('a')
        self.controller.instrument.outp_OFF('b')
        # self.controller.instrument.beeper(0.1)
        # self.controller.instrument.beeper(0.1)
        # print('unsubscribing measurement')
    ...
    def _getFromInst(self):
        smu_curr, smu_volt = self.controller.instrument.measure_channel(self.smu_chan)
        _, vm_volt = self.controller.instrument.measure_channel(self.vm_chan)
...
class I_V_Measurement(Measurement):
    ...
    def __Ax_Plot__(self):
        self.ax.plot([0, 1, 2], [4, 5, 6])
        ...
        if self.resmeas_bool == True:
            if self.resmeas_bool_polarity_switch == False:
                if self.iter == 0:
                    ...
                    smu_curr, _ = self.controller.instrument.measure_channel(self.smu_chan)
                    _, vm_volt = self.controller.instrument.measure_channel(self.vm_chan)
                    res = round(float(vm_volt) / float(smu_curr), 4)
