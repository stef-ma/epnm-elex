...
class InstCont_K2612B(tk.LabelFrame):
    ...
    def __init__(self, parent, controller):
        ...
        # We init the subframes inside top_frame as instances of MeasurementAndOutputSubframe_Dual_Channel. See below.
        self.chanA_Measframe = MeasurementAndOutputSubframe_Dual_Channel(lframeA, controller, chan_a, self)
        ...
        self.chanB_Measframe = MeasurementAndOutputSubframe_Dual_Channel(lframeB, controller, chan_b, self)
        ...
        self.multimeter_Measframe = MultimeterFrame_dualchan(multimeter_frame, controller, self)
...
class InstCont_K2400andK2182A(tk.LabelFrame):
    """The Manual Instrument Control Frame."""
    ...
    def __init__(self, parent, controller):
        ...
        # Set the controller as an attribute for easier handling.
        self.controller = controller
        self.instrument_SMU = self.controller.k2400_instrument
        self.single = True
        self.ON = False
        # Frames: OUTDATEDOUTDATEDOUTDATEDOUTDATEDOUTDATEDOUTDATEDOUTDATED
        #   |----------------InstCont_K2612B---------------|OUTDATED
        ...
        lframeA.grid(row=0, column=0, padx=10)
        # We init the subframes inside top_frame as instances of MeasurementAndOutputSubframe_Dual_Channel. See below.
        self.SMU_Measframe = MeasurementAndOutputSubframe_Single_Channel(lframeA, self.controller, self)
        self.SMU_Measframe.pack()

        # We init the subframes inside top_frame as instances of Channel_Subframe. See below.
        self.SMU_Subframe = SingleSubframe(top_frame, controller) ##### Needs to be replaced with Single Subframe
        self.SMU_Subframe.grid(row=0, column=1, padx=10)

        # self.NVolt_Subframe = ChannelSubframe(top_frame, chan_b, controller)
        # self.NVolt_Subframe.grid(row=0, column=2, padx=10)
        #
        lframeB = tk.LabelFrame(top_frame, width=controller.width / 5, height=controller.pane_height * 2, bg='black',
                                fg='white', bd=4, padx=10, pady=2, text='ChB')
        lframeB.grid(row=0, column=2, padx=10)
        lframeB.pack_propagate(0)
        #
        # self.NVolt_Measframe = MeasurementAndOutputSubframe_Dual_Channel(lframeB, controller, chan_b, self)
        # self.NVolt_Measframe.pack()


        # fr3 = NewFrame(lframeMEAS, controller.width/4-5, controller.pane_height * 3.5)
        # fr3.pack(side='left')

        fr3 = tk.LabelFrame(lframeB, width=controller.width / 4 - 5, height=controller.pane_height * 3.5, bg='black',
                            fg='white', bd=4, padx=10, pady=2, text='SETUP')
        fr3.grid(row=0, column=0)
        # fr2 = NewFrame(lframeSRC, controller.width/4-5, controller.pane_height * 3.5)
        fr4 = tk.LabelFrame(lframeB, width=controller.width / 4 - 5, height=controller.pane_height * 3.5, bg='black',
                            fg='white', bd=4, padx=10, pady=2, text='READ')
        fr4.grid(row=0, column=1)
        ...
        mid_frame.pack_propagate(0)
        mid_frame.pack(side='top')
        self.measurement_switcher = MeasurementSwitcher(mid_frame, self, 'Rt_k2400', 'IV', 'PLS_k2400')
        self.measurement_switcher.pack(fill='both', pady=2)
        self.measurement_switcher.pack_propagate(0)
        ...
        multimeter_frame.pack(side='top', anchor='s', padx=2)
        # multimeter_frame.pack_propagate(0)
        self.multimeter_Measframe = MultimeterFrame_singlechan(multimeter_frame, controller, self)
        self.multimeter_Measframe.pack()
    ...
    def readSettings(self):
        # Reads the settings for a given channel and updates the respective disabled entries in
        # chanX_Subframe via _updateControlFrame
        # func = self.instrument_SMU.get_SRC()
        if int(float(func)) == 0:  # for I src
            limit = self.instrument_SMU.get_limit_V()
            rng = self.instrument_SMU.get_range_I()
            level = self.instrument_SMU.get_level_I()
        elif int(float(func)) == 1:  # for V src
            limit = self.instrument_SMU.get_limit_I()
            rng = self.instrument_SMU.get_range_V()
            level = self.instrument_SMU.get_level_V()
        else:  # For unexpected errors.
            limit = 255
            rng = 255
            level = 255
        self._updateControlFrame(limit, rng, level, int(float(func)))

    def _updateControlFrame(self, limit, rng, level, func):
        # Updates the respective disabled entries in chanX_Subframe
        if func == 0:  # I src
            comp_unit = 'V'
            rng_unit = 'A'
            lvl_unit = 'A'
            self.SMU_Subframe.updateBlock(comp_unit, rng_unit, lvl_unit, limit, rng, level)
        elif func == 1:  # V src
            comp_unit = 'A'
            rng_unit = 'V'
            lvl_unit = 'V'
    ...
    def _switchButton(self, command):
        # Changes the  OUTPUT button between ON and OFF mode.
        chan_str = 'ChA'
        subframe = self.SMU_Measframe
        subframe.switchCommand(command)

    def _updateMeasurement(self):
        if self.ON:
            # Get data
            print(self.controller.k2400_instrument.measure())
            # print(float(self.controller.k2400_instrument.measure()))
            volt, curr, *_ = self.controller.k2400_instrument.measure()
            print('current is '+str(curr))
            print('voltage is '+str(volt))
            # Set subframe
            subframe = self.SMU_Measframe
...
class EntryFrame_K6221(tk.LabelFrame):
    def __init__(self, parent, what, controller):
        ...
        self.entry_stringvar.trace_add('write',
                                       lambda a, b, c: self.controller.controller_command_K6221(what, self.entry_stringvar))
...
class EntryFrame_K2182A(tk.LabelFrame):
    def __init__(self, parent, what, controller):
        ...
        self.entry.pack(side='left')
        self.entry_stringvar.trace_add('write',
                                       lambda a, b, c: self.controller.controller_command_K2182A(what, self.entry_stringvar))
        self.unit_stringvar = tk.StringVar(parent)
        if what == 'Rate':
            unit = 'NPLC'
        elif what == 'Range':
...
class ChannelSubframe(NewFrame):
    ...
    def __init__(self, parent, channel, controller):
        ...
        self.sour = InstContSubsMenus_multichan(frame, 'Sourcing', 'SRC', ['V', 'I'], channel, controller)
        self.sour.pack(side='left')
        # Create DropDowns for: Measuring
        self.meas = InstContSubsMenus_multichan(frame, 'Measuring', 'MEAS', ['V', 'I'], channel, controller)
        self.meas.pack(side='left')
        # Create DropDowns for: Sensing
        self.sens = InstContSubsMenus_multichan(frame, 'Sensing', 'SENS', ['2-Point', '4-Point'], channel, controller)
        self.sens.pack(side='left')
        # Create Entries for: Compliance
        what = 'Compliance'
        self.comp = InstContSubsEntries_multichan(self, what, self.sour.info, channel, controller)
        self.comp.pack(side='top')
        # Create Entries for: Range
        what = 'Range'
        self.rang = InstContSubsEntries_multichan(self, what, self.sour.info, channel, controller)
        self.rang.pack(side='top')
        # Create Entries for: Level
        what = 'Level'
        self.lvl = InstContSubsEntries_multichan(self, what, self.sour.info, channel, controller)
...
class SingleSubframe(NewFrame):
    ...
    def __init__(self, parent, controller):
        ...
        frame.pack(side='top')
        # Set the Label for the channel.
        label = tk.Label(frame, text='SOURCE', font=LARGE_FONT, bg='black', fg='white')
        label.pack(fill='x')
        # Create DropDowns for: Sourcing
        self.sour = InstContSubsMenus_singlechan(frame, 'Sourcing', 'SRC', ['V', 'I'], controller)
        self.sour.pack(side='left')
        # Create DropDowns for: Measuring
        self.meas = InstContSubsMenus_singlechan(frame, 'Measuring', 'MEAS', ['V', 'I'], controller)
        self.meas.pack(side='left')
        # Create DropDowns for: Sensing
        self.sens = InstContSubsMenus_singlechan(frame, 'Sensing', 'SENS', ['2-Point', '4-Point'], controller)
        self.sens.pack(side='left')
        # Create Entries for: Compliance
        what = 'Compliance'
        self.comp = InstContSubsEntries_singlechan(self, what, self.sour.info, controller)
        self.comp.pack(side='top')
        # Create Entries for: Range
        what = 'Range'
        self.rang = InstContSubsEntries_singlechan(self, what, self.sour.info, controller)
        self.rang.pack(side='top')
        # Create Entries for: Level
        what = 'Level'
        self.lvl = InstContSubsEntries_singlechan(self, what, self.sour.info, controller)
        self.lvl.pack(side='top')
...
class InstContSubsEntries_multichan(tk.LabelFrame):
    ...
    def __init__(self, parent, what, config_widget, chan, controller):
        ...
        self.entry.pack(side='left')
        self.entry_stringvar.trace_add('write',
                                       lambda a, b, c: self.controller.controller_command_K2612B(what, config_widget.get(), chan,
...
class InstContSubsEntries_singlechan(tk.LabelFrame):
    """Subframes of the Instrument Control Frame. Holds the entries for various configs."""

    def __init__(self, parent, what, config_widget, controller):
        # Init as LabelFrame for more efficient labelling
        tk.LabelFrame.__init__(self, parent, bg='black', fg='white', bd=5, padx=5, pady=5, text=what)
        self.configure(background='black')
        self.controller = controller  # alias the controller
        ...
        self.entry.pack(side='left')
        self.entry_stringvar.trace_add('write',
                                       lambda a, b, c: self.controller.controller_command_K2400(what,
                                                                                                config_widget.get(),
                                                                                                self.entry_stringvar)
                                       )
        # Create the StringVar to hold the program output and create the user indicator.
...
class InstContSubsMenus_multichan(tk.LabelFrame):
    ...
    def __init__(self, parent, text, what, options, chan, controller):
        ...
        self.ddown = tk.OptionMenu(self, self.info, *options,
                                   command=lambda x: self.controller.controller_config_K2612B(what, self.info.get(), chan))
        self.ddown.pack(side='bottom')


class InstContSubsMenus_singlechan(tk.LabelFrame):
    """Subframes of the Instrument Control Frame. Holds the OptionMenus for various configs."""

    def __init__(self, parent, text, what, options, controller):
        # Init as LabelFrame for more efficient labelling
        tk.LabelFrame.__init__(self, parent, bg='black', fg='white', bd=5, padx=5, pady=5, text=text)
        self.configure(background='black')
        self.controller = controller  # alias the controller
        ...
        self.info.set(what)
        self.ddown = tk.OptionMenu(self, self.info, *options,
                                   command=lambda x: self.controller.controller_config_K2400(what, self.info.get()))
        self.ddown.pack(side='bottom')

class MeasurementAndOutputSubframe_Single_Channel(NewFrame):
    ...
    def __init__(self, parent, controller, overframe):
        ...
        # On Button
        self.outp = tk.Button(bot_frame, text='ON', bg='orange', fg='white',
                              command=lambda: overframe.instContTurnOn())
        self.outp.pack(side='left', padx=2, pady=2)
        # Get Config Button
        self.read_conf = tk.Button(bot_frame, text='GET CONF', bg='gray', fg='white',
                                   command=lambda: overframe.readSettings())
        ...
        self.complianceWarning.makeText()

    def switchCommand(self, com):
        if com=='off':
            self.outp.config(text='OFF', bg='BLUE', fg='white',
                             command=lambda: self.overframe.instContTurnOff())
        if com=='on':
            self.outp.config(text='ON', bg='ORANGE', fg='white',
                             command=lambda: self.overframe.instContTurnOn())
...
class MeasurementAndOutputSubframe_Dual_Channel(NewFrame):
...
class MultimeterFrame_dualchan(NewFrame):
...
class MultimeterFrame_singlechan(NewFrame):
    ...
    def updateCallOut(self):
        source = self.overframe.SMU_Measframe.read_I_StringVar
        voltmeter = self.overframe.Ch1_read_V_StringVar # Hardcoded Ch1, should be selectable
...
class MeasurementSwitcher(tk.LabelFrame):
    ...
    def __init__(self, parent, overframe, string1='Rt', string2='IV', string3='PLS'):
        ...
        self.measure_button_Rt = tk.Button(frame1, text='R(t)', bg='gray', fg='white',
                                           command=lambda: self.controller.start_measurement(string1))
        #
        ...
        self.measure_button_IV = tk.Button(frame2, text='I(V)', bg='gray', fg='white', state='disabled',
                                           command=lambda: self.controller.start_measurement(string2))
        #
        ...
        self.measure_button_PulseRt = tk.Button(frame3, text='Pulse Sequence', bg='gray', fg='white',
                                                command=lambda: self.controller.start_measurement(string3))
        #
